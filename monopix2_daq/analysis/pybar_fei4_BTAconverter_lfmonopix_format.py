...
def process_dut(raw_data_file, output_filename=None, trigger_data_format=0, interpret_flag=True, format_flag=True, format_endstring='_interpreted.h5'):
    ...
    if interpret_flag==True: 
        analyze_raw_data(input_filename=raw_data_file, trigger_data_format=trigger_data_format)
    else:
        pass
    
    if format_flag==True:   
        output_filename = format_hit_table(input_filename=raw_data_filename + format_endstring, output_filename=output_filename)
    ...
    with tb.open_file(filename=input_filename, mode='r') as in_file_h5:
        ...
        with tb.open_file(filename=output_filename, mode='w') as out_file_h5:
            ...
            for read_index in tqdm(range(0, input_hits_table.nrows, chunk_size)):
                ...
                hits_data_formatted['tot'] = hits_chunk['tot']
                hits_data_formatted['phase'] = hits_chunk['TDC_time_stamp']
                hits_data_formatted['phase_quality'] = hits_chunk['TDC_trigger_distance']
...
if __name__ == "__main__":
    ...
    raw_data_files = [ "...ext_trigger_scan.h5"
                      ]
    trigger_data_format = 2
    # Flags to determine if the FE-I4 will be intepreted and/or formatted.
    # If any filter has been applied to interpreted FE data (e.g. during event building) the file only has to be formatted.
    interpret_flag=False
    format_flag=True
    # Endstring of a possibily modified interpreted file.
    format_endstring='_interpreted_valid.h5'
    # Simultaneous the processing of the files. The output is a formatted hit table.
    pool = Pool()
    results = [pool.apply_async(process_dut, kwds={'raw_data_file': raw_data_file, 'trigger_data_format': trigger_data_format, 'interpret_flag': interpret_flag
                                                   , 'format_flag': format_flag, 'format_endstring': format_endstring}) for raw_data_file in raw_data_files]
